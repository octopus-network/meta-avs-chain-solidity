...
import "@eigenlayer/src/contracts/interfaces/IBeaconChainOracle.sol";
...
import "@eigenlayer/src/contracts/pods/DelayedWithdrawalRouter.sol";

import "@eigenlayer/src/contracts/permissions/PauserRegistry.sol";

import "@eigenlayer/src/test/utils/Operators.sol";
...
import "@eigenlayer/src/test/mocks/BeaconChainOracleMock.sol";

import "forge-std/Test.sol";

contract EigenLayerDeployer is Operators {
    Vm _cheats = Vm(HEVM_ADDRESS);
    ...
    address sample_registrant = _cheats.addr(436364636);
    ...
    address acct_0 = _cheats.addr(uint256(priv_key_0));
    address acct_1 = _cheats.addr(uint256(priv_key_1));
    ...
    modifier fuzzedAddress(address addr) virtual {
        _cheats.assume(fuzzedAddressMapping[addr] == false);
        _;
    }

    modifier cannotReinit() {
        _cheats.expectRevert(bytes("Initializable: contract is already initialized"));
        _;
    }
    ...
    function setUp() public virtual {
        try vm.envUint("CHAIN_ID") returns (uint256 chainId) {
            if (chainId == 31337) {
                ...
                goerliDeploymentConfig = vm.readFile("script/output/M1_deployment_goerli_2023_3_23.json");
